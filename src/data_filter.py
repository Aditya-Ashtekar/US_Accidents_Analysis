...
spark = SparkSession.builder.master("local").appName("US accidents").getOrCreate()
...
mean_temperature = df7.agg({'Temperature(F)': 'mean'})
temp = mean_temperature.collect()

#for pressure taking mean
df8 = accident_df.select("Pressure(in)").where(col("Pressure(in)").isNotNull())
mean_pressure = df8.agg({'Pressure(in)': 'mean'})
pres = mean_pressure.collect()
